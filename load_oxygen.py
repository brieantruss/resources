...
def load_single_csv_to_mysql(db_config, file_path, table_name):
    ...
    try:
        ...
        with open(file_path, 'r', newline='') as csv_file:
            ...
            # Updated SQL INSERT statement for oxygen table with Last_Updated
            sql = f"INSERT INTO {table_name} (Date, Time, `o2_saturation`, Last_Updated) VALUES (%s, %s, %s, %s)"

            data_to_insert = []
            for row in csv_reader:
                # Expecting 4 columns: Date, Time, Oxygen saturation, Last_Updated
                if len(row) >= 4:
                    try:
                        date_str = row[0]
                        time_str = row[1]
                        oxygen_saturation_float = float(row[2]) # Changed to float for Oxygen saturation
                        last_updated_str = row[3] # New field

                        data_to_insert.append((date_str, time_str, oxygen_saturation_float, last_updated_str))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python3 load_oxygen.py <processed_files_base_directory>")
        sys.exit(1)
